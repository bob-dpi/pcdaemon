    ...
    SLOT *pslot)       // points to the SLOT for this plug-in
{
    ...
    if (pctx == (GAMEPAD *) 0) {
        ...
        pclog("memory allocation failure in gamepad initialization");
        return (-1);
    }
    ...
    if (pctx->period != 0)
        pctx->ptimer = add_timer(PC_PERIODIC, pctx->period, sendstate, (void *) pctx);
    else
        pctx->ptimer = (void *) 0;
    ...
    char    *buf)
{
    ...
    else if ((cmd == PCGET) && (rscid == RSC_FILTER)) {
        ret = snprintf(buf, *plen, "%05x\n", pctx->filter);
        *plen = ret;  // (errors are handled in calling routine)
    }
    else if ((cmd == PCGET) && (rscid == RSC_DEVICE)) {
    ...
    else if ((cmd == PCSET) && (rscid == RSC_FILTER)) {
    ...
    else if ((cmd == PCSET) && (rscid == RSC_DEVICE)) {
