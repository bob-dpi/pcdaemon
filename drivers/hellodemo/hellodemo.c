 ...
 *
 *  Resources:
 *    messagetext - the text of the message to broadcast (pcget, pcset)
 *    message - broadcast resource to hear message every period seconds (pccat)
 *    period  - update interval in seconds (pcget, pcset)
 */
    ...
    SLOT *pslot)       // points to the SLOT for this driver
{
    ...
    if (pctx == (HELLODEMO *) 0) {
        ...
        pclog("memory allocation failure in hellodemo initialization");
        return (-1);
    }
    ...
    pctx->ptimer = add_timer(PC_PERIODIC, (pctx->period * 1000), sendnow, (void *) pctx);
    ...
    char    *buf)
{
    ...
    if ((cmd == PCGET) && (rscid == RSC_PERIOD)) {
        ret = snprintf(buf, *plen, "%d\n", pctx->period);
        *plen = ret;  // (errors are handled in calling routine)
    }
    else if ((cmd == PCGET) && (rscid == RSC_TEXT)) {
    ...
    else if ((cmd == PCSET) && (rscid == RSC_TEXT)) {
