    ...
    SLOT *pslot)       // points to the SLOT for this plug-in
{
    ...
    if (pctx == (IRCCOM *) 0) {
        ...
        pclog("memory allocation failure in irccom initialization");
        return (-1);
    }
    ...
    char    *buf)
{
    ...
    else if ((cmd == PCGET) && (rscid == RSC_AVCHAN)) {
    ...
    else if ((cmd == PCGET) && (rscid == RSC_MYCHAN)) {
    ...
    else if ((cmd == PCSET) && (rscid == RSC_MYCHAN)) {
    ...
    IRCCOM  *pctx)     // our local info
{
    ...
    if (ret != 0) {
        ...
        pctx->ptimer = add_timer(PC_ONESHOT, ICM_RETRY, irc_connect, (void *) pctx);
        return;
    }
    ...
    for (resp = res; resp != NULL; resp = resp->ai_next) {
        ...
        if ((ret == -1) && (errno == EINPROGRESS)) {
            ...
            add_fd(pctx->ircfd, PC_WRITE, finish_connect, pctx);
            return;
        }
    ...
    IRCCOM  *pctx)     // our local info
{
    ...
    if (err == 0) {
        add_fd(pctx->ircfd, PC_READ, irc_receive, pctx);
    ...
    int      sndlen)   // number of characters in the buffer
{
    ...
    while (1) {
        ...
        pclog("Partial write in IRCCOM.  Retrying connection");
