...
locked satellites is available using pcget.  A stream
of location data is available using pccat.

config:
   The baudrate to use for the serial port and the
full path to the serial port's /dev entry.  The
config resource works with pcset and pcget.
...
Configure the system for 4800 baud and ttyUSB1
    pcset gps config 4800 /dev/ttyUSB1
