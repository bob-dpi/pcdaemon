 ...
 * 
 *  Resources:
 *    input      - 32 bit hex value at the inputs.  Works with pcget and pccat
    ...
    SLOT *pslot)       // points to the SLOT for this peripheral
{
    ...
    if (pctx == (in32DEV *) 0) {
        ...
        pclog("memory allocation failure in in32 initialization");
        return (-1);
    }
    ...
    int     len)       // number of bytes in the received packet
{
    ...
    if ((pkt->reg != IN32_R_PIN0) || (pkt->count != NPINS)) {
        pclog("invalid in32 packet from board to host");
        return;
    }
    ...
    char    *buf)
{
    ...
    PC_PKT   pkt;      // send write and read cmds to the in32
    ...
    if (rscid == RSC_INTR) {
        ...
        if (cmd == PCGET) {
            ...
            txret = pc_tx_pkt(pmycore, &pkt, 4);   // 4 header + 0 data for read
            if (txret != 0) {
