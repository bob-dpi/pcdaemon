 ...
 *
 *  Resources:
 *        current   - the most recent set of values (pcget)
 *        samples   - a stream of values (pccat)
 *
 *
    ...
    SLOT *pslot)       // points to the SLOT for this peripheral
{
    ...
    if (pctx == (SLIDE4DEV *) 0) {
        ...
        pclog("memory allocation failure in Slide4 initialization");
        return (-1);
    }
    ...
    int     len)       // number of bytes in the received packet
{
    ...
    else if ((pkt->cmd & PC_CMD_OP_MASK) == PC_CMD_OP_WRITE) {
    ...
    // error if not a data packet or an ACK
        ...
        pclog("invalid slide4 packet from board to host");
        return;
    }
}
    ...
    char    *buf)
{
    ...
    if ((cmd == PCGET) && (rscid == RSC_POSITIONS)) {
...
{
    PC_PKT   pkt;      // send write and read cmds to the adc812
