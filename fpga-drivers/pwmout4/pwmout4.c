    ...
    SLOT *pslot)       // points to the SLOT for this peripheral
{
    ...
    if (pctx == (PWM4DEV *) 0) {
        ...
        pclog("memory allocation failure in pwm4 initialization");
        return (-1);
    }
    ...
    int     len)       // number of bytes in the received packet
{
    ...
    pclog("invalid pwm4 packet from board to host");
    ...
    char    *buf)
{
    ...
    else if ((cmd == PCGET) && (rscid == RSC_PWMS)) {
    ...
    else if ((cmd == PCSET) && (rscid == RSC_PWMS)) {
...
{
    PC_PKT   pkt;      // send write and read cmds to the pwm4
    ...
    txret = pc_tx_pkt(pmycore, &pkt, 4 + pkt.count); // 4 header +  33 data
