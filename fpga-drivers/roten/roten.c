 ...
 *    led          - pcset to 0 or 1 to turn off or on the LED
 *
    ...
    SLOT *pslot)       // points to the SLOT for this peripheral
{
    ...
    if (pctx == (ROTENDEV *) 0) {
        ...
        pclog("memory allocation failure in roten initialization");
        return (-1);
    }
    ...
    int     len)       // number of bytes in the received packet
{
    ...
    if ((pkt->reg != ROTEN_REG_COUNT) || (pkt->count != 1)) {
        pclog("invalid roten packet from board to host");
        return;
    }
    ...
    ROTENDEV *pctx)    // this instance of the rotary encoder
{
    ...
    PC_PKT   pkt;      // send write and read cmds to the roten
    int      txret;    // ==0 if the packet went out OK
