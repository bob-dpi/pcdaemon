    ...
    SLOT *pslot)          // points to the SLOT for this peripheral
{
    ...
    if (pctx == (TOUCH4DEV *) 0) {
        ...
        pclog("memory allocation failure in touch4 initialization");
        return (-1);
    }
    ...
    int      len)       // number of bytes in the received packet
{
    ...
    if ((pkt->reg != COUNT4_REG_COUNT0) || (pkt->count != 16)) {
        pclog("invalid touch4 packet from board to host");
        return;
    }
    ...
    TOUCH4DEV *pctx)   // This peripheral's context
{
    ...
    txret = pc_tx_pkt(pmycore, &pkt, 4 + pkt.count); // 4 header +  2 data
