    ...
    SLOT *pslot)       // points to the SLOT for this peripheral
{
    ...
    if (pctx == (SERVO4DEV *) 0) {
        ...
        pclog("memory allocation failure in servo4 initialization");
        return (-1);
    }
    ...
    int     len)       // number of bytes in the received packet
{
    ...
    pclog("invalid servo4 packet from board to host");
...
{
    PC_PKT   pkt;      // send write and read cmds to the servo4
