    ...
    SLOT *pslot)       // points to the SLOT for this peripheral
{
    ...
    if (pctx == (STEPUDEV *) 0) {
        ...
        pclog("memory allocation failure in stepu initialization");
        return (-1);
    }
    ...
    int     len)       // number of bytes in the received packet
{
    ...
    if ((pkt->reg != STEPU_REG_COUNT) || (pkt->count != 2) ||
        ...
        pclog("invalid stepu packet from board to host");
        return;
    }

    // Must be a read response from a user pcget command, send value to UI
    ...
    char    *buf)
{
    ...
    if ((cmd == PCGET) && (rscid == RSC_COUNT)) {
        ...
        txret = pc_tx_pkt(pmycore, &pkt, 4);  // 4 header bytes
        if (txret != 0) {
    ...
    else if ((cmd == PCSET) && (rscid == RSC_COUNT)) {
    ...
    else if ((cmd == PCSET) && (rscid == RSC_ADD)) {
    ...
    else if ((cmd == PCSET) && (rscid == RSC_CNFG)) {
...
{
    PC_PKT   pkt;      // send write and read cmds to the stepu
    ...
    if (pctx->count != 0) {
        ...
        txret = pc_tx_pkt(pmycore, &pkt, 6); // 4 header + 2 data
    }
        ...
        txret = pc_tx_pkt(pmycore, &pkt, 7); // 4 header + 3 data
    }
