...
inputs : The value on the 4 pins
    A pcget on this resource returns the current values of all
4 pins.  A pcget requires a round trip over the USB-serial link
and may take a few milliseconds.  Pin values are given as a
hexadecimal number with the value of the lowest numbered pin
in the LSB.
   When used with pccat this resource reports the new value
