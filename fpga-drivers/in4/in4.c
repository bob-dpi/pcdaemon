 ...
 * 
 *  Resources:
 *    inputs    - values on all four pins as a hex digit (pcget/pccat)
 *    interrupt - interrupt-on-change mask (pcset/pcget)
 *
    ...
    SLOT *pslot)       // points to the SLOT for this peripheral
{
    ...
    if (pctx == (IN4DEV *) 0) {
        ...
        pclog("memory allocation failure in in4 initialization");
        return (-1);
    }
    ...
    int     len)       // number of bytes in the received packet
{
    ...
    if ((pkt->reg != IN4_REG_DATA) || (pkt->count != 1)) {
        pclog("invalid in4 packet from board to host");
    }
    ...
    char    *buf)
{
    ...
    // nothing to do if invoked to set up a pccat
    if (cmd == PCCAT) {
...
{
    int      txret;    // ==0 if the packet went out OK
    PC_PKT   pkt;      // send write and read cmds to the in4
