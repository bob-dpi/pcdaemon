    ...
    SLOT *pslot)       // points to the SLOT for this peripheral
{
    ...
    if (pctx == (GPIO4DEV *) 0) {
        ...
        pclog("memory allocation failure in gpio4 initialization");
        return (-1);
    }
    ...
    int     len)       // number of bytes in the received packet
{
    ...
    if ((pkt->reg != GPIO4_REG_PINS) || (pkt->count != 1)) {
        pclog("invalid gpio4 packet from board to host");
        return;
    }
    ...
    char    *buf)
{
    ...
    if (cmd == PCGET) {
        ...
        txret = pc_tx_pkt(pmycore, &pkt, 4 + pkt.count); // 4 header + 0 data on read
        if (txret != 0) {
