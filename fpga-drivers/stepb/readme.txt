...
This write-only resource works only with pcset.
...
current to 15 percent of full current.
    pcset stepb config half forward 40 15
    pcset stepb count 4000
    (sleep to let motor accelerate to 40 steps/sec)
    pcset stepb config half forward 80 15
    (sleep to let motor accelerate)
    pcset stepb config half forward 200 15
    (sleep or loop reading count until count < 2000)
    pcset stepb addstep 2000
