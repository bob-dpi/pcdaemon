    ...
    SLOT *pslot)       // points to the SLOT for this peripheral
{
    ...
    if (pctx == (STEPBDEV *) 0) {
        ...
        pclog("memory allocation failure in stepb initialization");
        return (-1);
    }
    ...
    int     len)       // number of bytes in the received packet
{
    ...
    if ((pkt->reg != STEPB_REG_COUNT) || (pkt->count != 2) ||
        ...
        pclog("invalid stepb packet from board to host");
        return;
    }
    ...
    char    *buf)
{
    ...
    if ((cmd == PCGET) && (rscid == RSC_COUNT)) {
        ...
        txret = pc_tx_pkt(pmycore, &pkt, 4); // 4 header
        if (txret != 0) {
...
{
    PC_PKT   pkt;      // send write and read cmds to the stepb
