 ...
 * slot 2:
 * 	# display 123456
 *      telnet localhost 8870
 * 	pcset 6 display 123456
 * 	# display 8.8.8.8.8
 * 	pcset 6 display 8.8.8.8.8
 ...
 * 	pcset 6 segments 80 80 80 60 60 60
 * 
 */ 
    ...
    int     len)       // number of bytes in the received packet
{
    ...
    pclog("invalid lcd6 packet from board to host");
    ...
    char    *buf)
{
    ...
    else if ((cmd == PCSET) && (rscid == RSC_SEGS )) {
    ...
    else if ((cmd == PCGET) && (rscid == RSC_DISP )) {
        ret = snprintf(buf, *plen, "%s\n", pctx->text);
        *plen = ret;  // (errors are handled in calling routine)
        return;
    }
    else if ((cmd == PCGET) && (rscid == RSC_SEGS )) {
...
{
    PC_PKT   pkt;      // send write and read cmds to the lcd6
    ...
    txret = pc_tx_pkt(pmycore, &pkt, LCD_PKT_LEN);
