 ...
 *
 *  Resources:
 *    recv         - broadcast (pccat ONLY) received IR packets in hex
 *    xmit         - 32 bit hex value to send.  Works with pcset only
 *
    ...
    SLOT *pslot)       // points to the SLOT for this peripheral
{
    ...
    if (pctx == (IRIODEV *) 0) {
        ...
        pclog("memory allocation failure in irio initialization");
        return (-1);
    }
    ...
    int     len)       // number of bytes in the received packet
{
    ...
    if ((pkt->reg != REG_DATA) || (pkt->count != 32)) {
        pclog("invalid irio packet from board to host");
        return;
    }
