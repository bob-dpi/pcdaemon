    ...
    SLOT *pslot)          // points to the SLOT for this peripheral
{
    ...
    if (pctx == (RCRXDEV *) 0) {
        ...
        pclog("memory allocation failure in rcrx initialization");
        return (-1);
    }
    ...
    int      len)       // number of bytes in the received packet
{
    ...
    if ( ! (   // if not a read response or autosend of timing
            ...
            ((pkt->reg == REG_RCDATA) && (pkt->count == 32) &&
              ((pkt->cmd & PC_CMD_AUTO_MASK) == PC_CMD_AUTO_DATA)))) {
        pclog("invalid rcrx packet from board to host");
        return;
    }
    ...
    char    *buf)
{
    ...
    if (rscid == RSC_GPIOVAL) {
        if (cmd == PCGET) {
            ...
            txret = pc_tx_pkt(pmycore, &pkt, 4); // header only on read request
            if (txret != 0) {
...
{
    PC_PKT   pkt;      // send write and read cmds to the rcrx  
