...
*    Command line: pcset avr fifo 0 5 6 7 8 9a bc de
...
int main()
{
    ...
    pcavr_register_fifo_set_hook(FifoSetHook);
     
    pcavr_println("fifo example");    
    while(1)
    {
        // pop and xmit a byte from the fifo if it's not empty
        if (!fifo_empty(&fifoTx))
        {
            pcavr_print_byte(fifo_pop(&fifoTx), 16);
            pcavr_print_string(" ");
        }
    }
