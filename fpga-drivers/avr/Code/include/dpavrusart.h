...
void pcavr_print_byte(unsigned char byte, int radix)
{
    if (radix == 10)
    {
        pcavr_tx_byte('0' + ((byte / 10) % 10));
        pcavr_tx_byte('0' + (byte % 10));
    }
    else if (radix == 16)
    {
...
// transmit a string
void pcavr_print_string(char* s)
{
    int i = 0;
    while (s[i])
    {
...
void pcavr_println(char* s)
{
    char* crlf = "\r\n";
    pcavr_print_string(s);
    pcavr_print_string(crlf);
}

// transmit a string, value, and newline
void pcavr_debug_print(char* s, unsigned char value, int radix)
{
    char* crlf = "\r\n";
    pcavr_print_string(s);
    pcavr_print_byte(value, radix);
    pcavr_print_string(crlf);
}
