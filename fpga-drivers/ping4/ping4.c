 ...
 * 
 *  Resources:
 *    distance   - sensor ID and its distance (works with pccat)
 *    enable     - bit mask to enable (1) or disable (0) a sensor (pcset/pcget)
 *
    ...
    SLOT *pslot)       // points to the SLOT for this peripheral
{
    ...
    if (pctx == (PING4DEV *) 0) {
        ...
        pclog("memory allocation failure in ping4 initialization");
        return (-1);
    }
    ...
    int     len)       // number of bytes in the received packet
{
    ...
    if (((pkt->cmd & PC_CMD_AUTO_MASK) == PC_CMD_AUTO_DATA) &&
        ((pkt->cmd & PC_CMD_OP_MASK) == PC_CMD_OP_READ) &&
    ...
    if ((pkt->reg != PING4_REG_TIME) || (pkt->count != 3)) {
        pclog("invalid ping4 packet from board to host");
    }
    ...
    char    *buf)
{
    ...
    if ((cmd == PCGET) && (rscid == RSC_ENABLE)) {
        ret = snprintf(buf, *plen, "%d\n", pctx->enable);
        *plen = ret;  // (errors are handled in calling routine)
    }
    // Write of the enable value?
    else if ((cmd == PCSET) && (rscid == RSC_ENABLE)) {
    ...
    PING4DEV *pctx)    // our device instance
{
    ...
    PC_PKT   pkt;      // send write and read cmds to the ping4
