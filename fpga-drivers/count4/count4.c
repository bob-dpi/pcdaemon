    ...
    SLOT *pslot)          // points to the SLOT for this peripheral
{
    ...
    if (pctx == (COUNT4DEV *) 0) {
        ...
        pclog("memory allocation failure in count4 initialization");
        return (-1);
    }
    ...
    int      len)       // number of bytes in the received packet
{
    ...
    if ((pkt->reg != COUNT4_REG_COUNT0) || (pkt->count != 16)) {
        pclog("invalid count4 packet from board to host");
        return;
    }
...
{
    PC_PKT   pkt;      // send write and read cmds to the count4
