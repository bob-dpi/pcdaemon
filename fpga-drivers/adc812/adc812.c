 ...
 *    This is a read-write resource that works with pcset * and pcget
 * but not pccat.
 *
 ...
 * bit result.  You can use pccat with this resource
 */
    ...
    SLOT *pslot)       // points to the SLOT for this peripheral
{
    ...
    if (pctx == (ADC812DEV *) 0) {
        ...
        pclog("memory allocation failure in adc812 initialization");
        return (-1);
    }
    ...
    int     len)       // number of bytes in the received packet
{
    ...
    if ((pkt->reg != ADC812_REG_ADC0) || (pkt->count != 16)) {
        pclog("invalid adc812 packet from board to host");
        return;
    }
