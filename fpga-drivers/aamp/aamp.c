    ...
    SLOT *pslot)    // points to the SLOT for this peripheral
{
    ...
    if (pctx == (AAMPDEV *) 0) {
        ...
        pclog("memory allocation failure in aamp initialization");
        return (-1);
    }
    ...
    pctx->volTimer = add_timer(PC_PERIODIC, VOLUME_CHANGE_DELAY, VolumeToFpga, (void *) pctx);
    ...
    int      len)       // number of bytes in the received packet
{
    ...
    // Error if not a write response packet
        ...
        pclog("invalid aamp packet from board to host");
        return;
    }
    ...
    if (rscid == RSC_VOLUME) 
    {
        ...
        else 
        {
            ...
            if (pctx->enabledState == STATE_ON)
            {
                ...
                if (pctx->volTimer == 0)
                {
                    pctx->volTimer = add_timer(PC_PERIODIC, 100, VolumeToFpga, (void *) pctx);
                }
            }
        }
        return;
...
static void VolumeToFpga(void *timer, AAMPDEV *pctx)
{
    ...
    if (pctx->volume != pctx->targetVolume)
    {
        ...
        pc_tx_pkt(pmycore, &pkt, 5);
        if (pctx->ptimer == 0)
        {
            pctx->ptimer = add_timer(PC_ONESHOT, VOLUME_CHANGE_DELAY, noAck, (void *) pctx);
        }
...
{
    PC_PKT   pkt;      // send write and read cmds to the aamp
    SLOT    *pmyslot;  // This peripheral's slot info
    CORE    *pmycore;  // FPGA peripheral info
    int      txret;    // ==0 if the packet went out OK
