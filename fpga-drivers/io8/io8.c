 ...
 * 
 *  Resources:
 *    output       - 8 bit hex value of the outputs. Works with pcget and pcset
 *    input        - 8 bit hex value at the inputs.  Works with pcget and pccat
    ...
    SLOT *pslot)       // points to the SLOT for this peripheral
{
    ...
    if (pctx == (IO8DEV *) 0) {
        ...
        pclog("memory allocation failure in io8 initialization");
        return (-1);
    }
    ...
    int     len)       // number of bytes in the received packet
{
    ...
    if ((pkt->reg != IO8_R_PIN0) || (pkt->count != 8)) {
        pclog("invalid io8 packet from board to host");
        return;
    }
    ...
    char    *buf)
{
    ...
    PC_PKT   pkt;      // send write and read cmds to the io8
    ...
    if (rscid == RSC_OUTPUT) {
    ...
    else {          // must be pcset or pccat for inputs
        if (cmd == PCGET) {
        ...
        // else = nothing.  No action needed for pccat of inputs
    }
    ...
    char    *buf)      // where to store user visible error messages
{
    ...
    txret = pc_tx_pkt(pmycore, &pkt, 12); // 4 header + 8 data
