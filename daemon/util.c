 ...
 *              the log utilities for pcdaemon.
 *
...
extern PC_FD     Pc_Fd[];   // Array of open FDs and callbacks
extern PC_TIMER  Timers[];  // Array of timers and callbacks
extern char     *CmdName;
...
/***************************************************************************
 *  muxmain():  The main pcdaemon event loop.
 ***************************************************************************/
void muxmain()
{
    struct timeval *ptv;
    ...
    void    *pcb_data)  // callback data 
{
    ...
    // Find the first free entry in Pc_Fd
    for (i = 0; i < MX_FD; i++) {
        if (Pc_Fd[i].fd == -1) {
            pinfo = &Pc_Fd[i];
            break;
    ...
    // At this point we've walked the PC_FD array and have
    // found an empty PC_FD.  Add the new entry.
    pinfo->fd = fd;
    pinfo->stype = stype;
    pinfo->scb = scb;
...
void update_fdsets()
{
    ...
    for (i = 0; i < MX_FD; i++) {
        ...
        fdcount++;
        mxfd = (Pc_Fd[i].fd > mxfd) ? Pc_Fd[i].fd : mxfd;
        if ((Pc_Fd[i].stype & PC_READ) != 0)
            FD_SET(Pc_Fd[i].fd, &gRfds);
        if ((Pc_Fd[i].stype & PC_WRITE) != 0)
            FD_SET(Pc_Fd[i].fd, &gWfds);
        if ((Pc_Fd[i].stype & PC_EXCEPT) != 0)
            FD_SET(Pc_Fd[i].fd, &gXfds);
    }
    return;
}
...
/***************************************************************************
 *  pclog():  Print logmessages to stderr or syslog
 ***************************************************************************/
void pclog(
 ...
 * type is PC_PERIODIC and will occur once if PC_ONESHOT.
 ...
 * them (PC_ONESHOT) or reschedule them (PC_PERIODIC).
...
struct timeval *doTimer()
{
    ...
    for (i = 0; i < MX_TIMER; i++) {
        ...
        if (Timers[i].type == PC_PERIODIC) { /* Periodic, so reschedule */
    ...
    if (nextto == -1) {
        ...
        pclog("pcdaemon internal timer error");
        return ((struct timeval *) 0);
    }
 ...
 *
 * Input:        int   type -- PC_ONESHOT or PC_PERIODIC
    ...
    void    *pcb_data)  // callback data
{
    ...
    /* Sanity checks */
    if (cb == (void *) 0) {
        pclog("Adding timer with null callback");
        return((void *) 0);
    }
    if (ms == 0 && type == PC_PERIODIC) {
        pclog("Periodic timer with period = 0");
        return ((PC_TIMER *) 0);
    }
    ...
    /* OK, we've got the PC_TIMER struct, now fill it in */
    ntimers++;       /* increment number of PC_TIMER structs alloc'ed */
    ...
    void    *ptimer)
{
    ...
    if (((ptimer - (void *) &Timers[0]) % sizeof(PC_TIMER)) != 0) {
        return;
    }
    if (((PC_TIMER *) ptimer)->type == PC_UNUSED) {
        return;
    }

    ((PC_TIMER *) ptimer)->type = PC_UNUSED;
