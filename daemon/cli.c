 ...
 * Name: cli.c
 *
 * Description: A utility that issues commands to the pcdaemon using
...
#define MAX_IP       50
        // Max length of cmd down to the pcdaemon
#define MAX_PCCMD   250
...
/**************************************************************
 * cli(): - Send commands to an pcdaemon.  This program lets
...
int main(int argc, char **argv)
{
    ...
    int  bindport;          // TCP port to the pcdaemon
    int  cmdc;              // command line character option
    int  tmp_int;           // a temporary integer
    static int srvfd = -1;  // FD for pcdaemon socket
    struct sockaddr_in skt; // network address for pcdaemon
    int  adrlen;
    int  i;                 // generic loop counter
    int  ret;               // generic return value
    char buf[MAX_PCCMD];    // command to send to daemon
    int  slen;              // length of string in buf
    int  nout;              // num chars written to pcdaemon
    char c;                 // response character from pcdaemon
    ...
    if ((inet_aton(bindaddress, &(skt.sin_addr)) == 0) ||
        ...
        printf("Error: unable to connect to the pcdaemon.\n");
        exit(-1);
    }

    // At this point the connection to the pcdaemon is open
    ...
    for (i = optind; (i < argc && slen < MAX_PCCMD -1) ; i++) {
        slen += snprintf(&(buf[slen]), (MAX_PCCMD - slen), "%s ", argv[i]);
    }
    if (slen >= MAX_PCCMD -1) {
    ...
    // write complete command to the pcdaemon
    nout = 0;
    while (nout != slen) {
        ...
        if ((ret < 0) && (errno == EAGAIN))
            ...
            printf("Error writing to pcdaemon\n");
            exit(-1);
        }
    ...
    while(1) {
        ...
        if ((c == PROMPT) || (ret == 0)) {
            ...
            printf("\nError reading from pcdaemon connection\n");
            exit(-1);
        }
        if (errno == EAGAIN) {
            continue;
...
char usagetext[] = "\
Usage is command specific.  pcdaemon command syntaxes are as follows:\n\
